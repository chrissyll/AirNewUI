//------------------------------------------------------------------------------
// <auto-generated>
//     這個程式碼是由範本產生。
//
//     對這個檔案進行手動變更可能導致您的應用程式產生未預期的行為。
//     如果重新產生程式碼，將會覆寫對這個檔案的手動變更。
// </auto-generated>
//------------------------------------------------------------------------------

namespace AirTicket
{
    using System;
    using System.Collections.Generic;
    
    public partial class Order
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public Order()
        {
            this.Passengers = new HashSet<Passenger>();
        }
    
        public int Ticket_ID { get; set; }
        public Nullable<int> Airline_ID { get; set; }
        public Nullable<int> Flight_ID { get; set; }
        public Nullable<System.DateTime> Departure_Time { get; set; }
        public Nullable<int> Departure_Country_ID { get; set; }
        public Nullable<int> Departure_Airport_ID { get; set; }
        public Nullable<int> Arrival_Country_ID { get; set; }
        public Nullable<int> Arrival_Airport_ID { get; set; }
        public Nullable<System.DateTime> Arrival_Time { get; set; }
        public Nullable<int> Cabin_ID { get; set; }
        public Nullable<int> Member_ID { get; set; }
        public Nullable<bool> Insurance { get; set; }
        public string Payment { get; set; }
        public string NonStop { get; set; }
        public Nullable<System.DateTime> Order_Time { get; set; }
    
        public virtual Airline Airline { get; set; }
        public virtual Flight Flight { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<Passenger> Passengers { get; set; }
        public virtual Airport Airport { get; set; }
        public virtual Airport Airport1 { get; set; }
        public virtual Cabin Cabin { get; set; }
    }
}
